...
def process_encoding_input(target_data, vocab2int, batch_size):
    ...
    dec_input = tf.concat(
        [tf.fill([batch_size, 1], vocab2int['<GO>']), ending], 1)
...
def training_decoding_attention(rnn_size, enc_output, enc_state, input_length,
    ...
    initial_state_training = dec_cell_training.zero_state(
        batch_size, tf.float32)
    initial_state_training = initial_state_training.clone(
...
def inference_decoding_attention(enc_output, enc_state, input_length, rnn_size, dec_cell,
    ...
    """Prepare the inference attention and initial state"""
    tiled_encoder_outputs = tf.contrib.seq2seq.tile_batch(
        enc_output, multiplier=beam_width)
    tiled_encoder_final_state = tf.contrib.seq2seq.tile_batch(
        enc_state[0], multiplier=beam_width)
    tiled_sequence_length = tf.contrib.seq2seq.tile_batch(
        input_length, multiplier=beam_width)
    ...
    decoder_initial_state_inference = decoder_initial_state_inference.clone(
