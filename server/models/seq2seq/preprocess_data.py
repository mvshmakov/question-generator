...
def save_pickle(data, filename):
    ...
    save_documents = open('data/sberquad/pickles/'+filename+'.pickle', 'wb')
...
def load_pickle(data_filepath):
    ...
    documents_f = open('data/sberquad/pickles/'+data_filepath+'.pickle', 'rb')
...
def preprocess_data(data, remove_stopwords=True, replace_entities=False):
    ...
    for index in range(len(data)):
        ...
        if replace_entities:
            ...
            spacy_text = nlp(text)
            text_ents = [(str(ent), str(ent.label_))
...
def create_embedding_matrix(vocab2int, embeddings_index, embedding_dimensions=300):
    ...
    word_embedding_matrix = np.zeros(
        (num_words, embedding_dimensions), dtype=np.float32)
    for token, index in vocab2int.items():
        if token in embeddings_index:
            ...
            new_embedding = np.array(
                np.random.uniform(-1.0, 1.0, embedding_dimensions))
...
def preprocess_dataset():
    ...
    # ==============================================================================
    vocab2int, int2vocab = create_conversion_dictionaries(
        word_frequency, embeddings_index)
    ...
    # ==============================================================================
    word_embedding_matrix = create_embedding_matrix(
        vocab2int, embeddings_index)
    ...
    sorted_inputs, sorted_targets = remove_wrong_length_data(converted_inputs,
                                                             converted_targets,
                                                             vocab2int,
                                                             start_inputs_length=min(
                                                                 summary_inputs.counts),
                                                             max_inputs_length=int(np.percentile(
                                                                 summary_inputs.counts, 100)),
                                                             max_targets_length=int(np.percentile(
                                                                 summary_targets.counts, 100)),
